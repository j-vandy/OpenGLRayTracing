...
Camera camera(45, 0.01f, 100.0f, window_width, window_height);
double deltaTime;
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods) {
	camera.HandleKeyInput(deltaTime, action, key);
}

// cursor position
void cursor_position_callback(GLFWwindow* window, double xpos, double ypos) {
	camera.HandleMousePositionInput(deltaTime, xpos, ypos);
}
...
int main()
{
	...
	// set the framebufer size callback
	...
	{
		// calculate deltaTime between updates
